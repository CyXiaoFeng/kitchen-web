        ...
        >
          ...
          <template v-slot:body-cell-OrderNo="props">
            <q-td :props="props">
              <a
                href="javascript:"
                @click.prevent="goPage('/order-detail/' + props.row.order_id)"
                >{{ props.value }}</a
              >
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-Action="props">
            <q-td style="max-width: 150px; min-width: 100px" :props="props">
              <q-btn
...
export default {
  ...
  methods: {
    ...
    finishOrder() {
      this.$api
        .put("/api/v1/table/finish/" + this.table_id)
        .then((response) => {
          if (response.data.ok === true) {
            this.notifySuccess(response.data.message)
          } else {
    ...
    assignItemsByTable() {
      ...
      this.$api
        .post("/api/v1/assignment/" + activeOrderId)
        .then((response) => {
          if (response.data.ok === true) {
            this.notifySuccess(response.data.message)
            setTimeout(() => {
    ...
    loadOrderItemsByTableId(tableId, pageSize) {
      ...
      this.$api
        .get("/api/v1/order-item/active-item-of-table", {
          params: params,
        })
        .then((response) => {
